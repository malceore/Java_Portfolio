...
public class AvaRules implements Rule {
    ...
    private HouseMateEntitler titler;

    public AvaRules(HouseMateEntitler titler, AccessToken token){
        this.token = token;
        this.titler = titler;
    }
    ...
    public boolean execute(String change, HouseMateModel model) {
        ...
        if (change.contains("voice_command")) {
            ...
            // Can check voice command but currently test file has no voice prints.
            if (true) { //titler.voicePrintLookUp(print)
                //Where
